package org.grpc.plant.config;

import org.grpc.plant.service.RobotCustomiseServiceImpl;
    ...
    public void start() {
        try {
            server = ServerBuilder.forPort(grpcPort)
                    .addService(new RobotCustomiseServiceImpl())
                    .build()
                    .start();

            running = true;
            log.info("gRPC-сервер запущен на порту {}", grpcPort);
            log.info("Сервис: RobotCustomise.editRobot()");
